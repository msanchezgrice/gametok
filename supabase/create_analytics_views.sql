...
WITH session_metrics AS (
  SELECT
    ...
    MIN(gs.created_at) as first_played_at
...
ON public.game_sessions (game_id, created_at, completed, user_id);
...
BEGIN
  ...
  WITH recent_sessions AS (
    ...
    FROM public.game_sessions gs
    WHERE gs.created_at >= NOW() - INTERVAL '1 day' * time_range_days
    GROUP BY gs.game_id
  ),
  recent_favorites AS (
    SELECT
...
-- Test the view
SELECT * FROM public.game_analytics_summary LIMIT 10;